...
*Setup instructions for a Gitian build of biggcoincoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the biggcoincoin
...
- [Building biggcoincoin Core](#building-biggcoincoin-core)
...
Clone the git repositories for biggcoincoin and Gitian.
...
in the biggcoincoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building biggcoincoin Core
----------------

To build biggcoincoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the biggcoincoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/biggcoincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/biggcoincoin-project/biggcoincoin
...
For example:
```bash
URL=https://github.com/thrasher-/biggcoincoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit biggcoincoin=${COMMIT} --url biggcoincoin=${URL} ../biggcoincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit biggcoincoin=${COMMIT} --url biggcoincoin=${URL} ../biggcoincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit biggcoincoin=${COMMIT} --url biggcoincoin=${URL} ../biggcoincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the biggcoincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../biggcoincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/biggcoincoin
SIGPATH=/some/root/path/biggcoincoin-detached-sigs

./bin/gbuild --url biggcoincoin=${BTCPATH},signature=${SIGPATH} ../biggcoincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/biggcoincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/biggcoincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/biggcoincoin-osx-build.assert
```
...
[biggcoincoin-project/gitian.sigs.ltc](https://github.com/biggcoincoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
